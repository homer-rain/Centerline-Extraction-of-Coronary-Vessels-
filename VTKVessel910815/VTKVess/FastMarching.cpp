...
int FastMarching:: Segment(InternalImageType::Pointer image, InternalImageType::IndexType  seedPosition  )
{
  ...
  const InternalPixelType  timeThreshold = atof( "1" );
  ...
  // Software Guide : BeginCodeSnippet
  smoothing->SetInput( image/*reader->GetOutput()*/ );
...
//  InternalImageType::IndexType  seedPosition;
 ...
 // seedPosition[0] = atoi( argv[3] );
  ...
  const double stoppingTime = atof( "3" );
 ...
 // writer4->Update();
  ...
  mapWriter->SetFileName("FastMarchingFilterOutput4.mhd");
  mapWriter->Update();

  InternalWriterType::Pointer speedWriter = InternalWriterType::New();
  speedWriter->SetInput( sigmoid->GetOutput() );
  speedWriter->SetFileName("FastMarchingFilterOutput3.mhd");
  speedWriter->Update();

  InternalWriterType::Pointer gradientWriter = InternalWriterType::New();
  gradientWriter->SetInput( gradientMagnitude->GetOutput() );
  gradientWriter->SetFileName("FastMarchingFilterOutput2.mhd");
