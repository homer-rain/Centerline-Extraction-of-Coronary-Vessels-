 ...
 ::GenerateData()
 {
   ...
   if ( m_SafeBorder )
     ...
     progress->RegisterInternalFilter(pad, .f);
     progress->RegisterInternalFilter(erode, .f);
     progress->RegisterInternalFilter(dilate, .f);
     progress->RegisterInternalFilter(crop, .f);
